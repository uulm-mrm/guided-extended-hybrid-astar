...
private:
  ...
  inline static constexpr int MAX_INTERPOLATION_DEG = 3;
  ...
  inline static std::array<int, NB_DIR> direction_inputs_;
  ...
  inline static double comfort_brake_acc_;
  ...
  inline static int max_dist4waypoints_;
...
public:
  ...
  static void calcConnectedClosedNodes();

  static Path getEmergencyPath(const NodeHybrid& start_node, double ego_v);
...
private:
  ...
  static double getRAPathCosts(const ReedsSheppStateSpace::ReedsSheppPath& path);

  static double getRSPathCosts(const ReedsSheppStateSpace::ReedsSheppPath& path);

  static ReedsSheppStateSpace::ReedsSheppPath
  getReedSheppPath(const Pose<double>& start, const Pose<double>& goal, double step_size, double rho, double wb);
  ...
  static std::optional<NodeHybrid>
  hAstarCore(const NodeHybrid& ego_node, const NodeHybrid& start_node, const NodeHybrid& goal_node, bool do_analytic);
