                                                      ...
                                                      double minDist)
{
  ...
  vdg.generateVoronoi(samples, numPoints, minX, maxX, minY, maxY, minDist);
  vdg.resetIterator();
...
struct Halfedge* VoronoiDiagramGenerator::ELleftbnd(struct Point* p)
{
  ...
  bucket = (int)((p->x - xmin) / deltax * ELhashsize);  // use the hash function to find the place in the hash map that
                                                        // this HalfEdge should be
...
void VoronoiDiagramGenerator::geominit()
{
  ...
  nvertices = 0;
  nedges = 0;
  sn = static_cast<double>(nsites) + 4;
...
void VoronoiDiagramGenerator::plotinit()
{
  ...
  dy = ymax - ymin;
  dx = xmax - xmin;
  d = static_cast<double>((dx > dy ? dx : dy) * 1.1);
  pxmin = static_cast<double>(xmin - (d - dx) / 2.0);
  pxmax = static_cast<double>(xmax + (d - dx) / 2.0);
  pymin = static_cast<double>(ymin - (d - dy) / 2.0);
  pymax = static_cast<double>(ymax + (d - dy) / 2.0);
  cradius = static_cast<double>((pxmax - pxmin) / 350.0);
  openpl();
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
  ...
  while (1)
  {
    ...
    else if (!PQempty()) /* intersection is smallest - this is a vector event */
    {
      ...
      ELdelete(rbnd);  // mark the right HE for deletion - can't delete yet because there might be pointers to it in
                       // Hash Map
